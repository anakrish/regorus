...
if [ "$DIFF_FULL_SIZE" -gt 30000 ]; then
  echo "::warning::Diff is ${DIFF_FULL_SIZE} bytes (>30KB). Quick review cannot fit this in a single model context. Use deep review mode instead (trigger via ready_for_review or workflow_dispatch with mode=deep)."
  echo "::notice::Skipping quick review — PR too large for single-shot analysis. Deep review recommended."
  rm -f "$TMPDIR/pr_diff_full.txt"
  exit 0
fi
# Truncate diff to 15KB to fit within model token limits alongside anchors+knowledge
DIFF_TRUNCATED=false
if [ "$DIFF_FULL_SIZE" -gt 15000 ]; then
  DIFF_TRUNCATED=true
  echo "::warning::Diff truncated from ${DIFF_FULL_SIZE} to 15000 bytes. Files/hunks beyond the cutoff are NOT reviewed. Consider deep review for full coverage."
fi
head -c 15000 "$TMPDIR/pr_diff_full.txt" > "$TMPDIR/pr_diff.txt"
rm -f "$TMPDIR/pr_diff_full.txt"
echo "Diff size: $(wc -c < "$TMPDIR/pr_diff.txt") bytes (from ${DIFF_FULL_SIZE})"
...
# Build a compact anchor table for the prompt (file:line only to save tokens).
ANCHOR_TABLE=$(awk -F: '
  $3 == "added" { print "  + " $1 ":" $2 }
' "$TMPDIR/valid_anchors_full.txt" | head -200)

# Context lines — compact file:line
CONTEXT_SUMMARY=$(awk -F: '
  $3 == "context" { print $1 ":" $2 }
' "$TMPDIR/valid_anchors_full.txt" | head -20)
...
for perspective in $(echo "$PERSPECTIVES" | tr ',' ' '); do
  ...
  if [ -f "$agent_file" ]; then
    AGENT_INSTRUCTIONS=$(head -c 2000 "$agent_file")
  fi
  ...
  # Allow critical findings through (genuine factual errors in docs should still be reported).
  if [ "$perspective" != "ci-cd-security" ]; then
    CONTENT=$(echo "$CONTENT" | jq -c '
      if type == "array" then
        [.[] | select(
          (.file // "") as $f |
          ($f | (endswith(".agent.md") or endswith("SKILL.md") or startswith("docs/knowledge/")))
          | if . then (.severity == "critical") else true end
        )]
      else . end')
  fi
...
> ⚠️ **Partial review**: Diff was truncated to 15KB (full: ${DIFF_FULL_SIZE} bytes). Files beyond the cutoff were not reviewed."
  fi
