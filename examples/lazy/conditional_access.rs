...
fn main() -> Result<()> {
    ...
    impl FieldGetter for DetailsGetter {
        fn get(&self, _ctx: &LazyContext) -> Result<Value> {
            ...
            obj.as_object_mut()?
                .insert(Value::from("verified"), Value::Bool(true));
            Ok(obj)
        }
    }
    ...
    impl FieldGetter for MetadataGetter {
        fn get(&self, _ctx: &LazyContext) -> Result<Value> {
            ...
            let mut obj = Value::new_object();
            obj.as_object_mut()?
                .insert(Value::from("owner"), Value::from("alice"));
            Ok(obj)
        }
    }
    ...
    SchemaBuilder::new("Resource")
        ...
        .field_immediate(
            "details",
            DetailsGetter {
                count: details_count.clone(),
            },
        )
        .field_immediate(
            "metadata",
            MetadataGetter {
                count: metadata_count.clone(),
            },
        )
        .register();
    ...
    let lazy_obj = LazyObject::new(TypeId::new("Resource"), LazyContext::new());
    ...
    println!(
        "\n✅ Result: {}",
        results.result.len() > 0 && results.result[0].expressions.len() > 0
    );
    println!("\n📊 Access Statistics:");
    println!(
        "  - details: {} times",
        details_count.load(Ordering::SeqCst)
    );
    println!(
        "  - metadata: {} times (should be 0!)",
