...
fn main() -> Result<()> {
    ...
    impl IndexGetter for UsersIndexGetter {
        fn get_at(&self, _ctx: &LazyContext, index: usize) -> Result<Option<Value>> {
            if let Some(user_id) = self.db.get_user_at_index(index)? {
                ...
                let schema_name: &'static str =
                    Box::leak(format!("User{}", user_id).into_boxed_str());
    ...
    input1
        .as_object_mut()?
        .insert(Value::from("user"), Value::LazyObject(Arc::new(user1)));
    ...
    println!(
        "\n✅ Is admin: {}",
        result1.result.len() > 0 && result1.result[0].expressions.len() > 0
    );
    println!(
        "📊 SQL queries executed: {} (only queried 'role' field!)",
        db.get_query_count()
    );
    ...
    input2.as_object_mut()?.insert(
        Value::from("users"),
        Value::LazyArray(Arc::new(users_array)),
    );
    ...
    input3
        .as_object_mut()?
        .insert(Value::from("user"), Value::LazyObject(Arc::new(user3)));
    ...
    println!(
        "\n✅ Has high quota: {}",
        result3.result.len() > 0 && result3.result[0].expressions.len() > 0
    );
    println!(
        "📊 SQL queries executed: {} (only queried 'quota' field!)",
        db.get_query_count()
    );
    ...
    input4
        .as_object_mut()?
        .insert(Value::from("user"), Value::LazyObject(Arc::new(user4)));
    ...
    println!(
        "\n✅ Allowed: {}",
        result4.result.len() > 0 && result4.result[0].expressions.len() > 0
    );
    println!(
        "📊 SQL queries executed: {} (early exit on admin check!)",
        db.get_query_count()
    );
