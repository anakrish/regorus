...
fn main() -> Result<()> {
    ...
    impl FieldGetter for AuthGetter {
        fn get(&self, _ctx: &LazyContext) -> Result<Value> {
            ...
            auth.as_object_mut()?
                .insert(Value::from("authenticated"), Value::Bool(true));
            auth.as_object_mut()?
                .insert(Value::from("principal"), Value::from("user@example.com"));
            Ok(auth)
        }
    }
    ...
    impl FieldGetter for QuotaGetter {
        fn get(&self, _ctx: &LazyContext) -> Result<Value> {
            ...
            quota
                .as_object_mut()?
                .insert(Value::from("limit"), Value::from(1000));
            quota
                .as_object_mut()?
                .insert(Value::from("used"), Value::from(1000 - self.remaining));
            quota
                .as_object_mut()?
                .insert(Value::from("remaining"), Value::from(self.remaining));
            Ok(quota)
        }
    }
    ...
    SchemaBuilder::new("Validator1")
        ...
        .field_immediate(
            "auth",
            AuthGetter {
                count: auth_count1.clone(),
            },
        )
        .field_immediate(
            "quota",
            QuotaGetter {
                count: quota_count1.clone(),
                remaining: 650,
            },
        )
        .register();
    ...
    input1.as_object_mut()?.insert(
        Value::from("validation"),
        Value::LazyObject(Arc::new(validator1)),
    );
    ...
    println!(
        "  - Stage 2 (Auth): {} checks",
        auth_count1.load(Ordering::SeqCst)
    );
    println!(
        "  - Stage 3 (Quota): {} checks",
        quota_count1.load(Ordering::SeqCst)
    );
    ...
    SchemaBuilder::new("Validator2")
        ...
        .field_immediate_fn("path", |_| Ok(Value::from("/api/v1/resources")))
        .field_immediate(
            "auth",
            AuthGetter {
                count: auth_count2.clone(),
            },
        )
        .field_immediate(
            "quota",
            QuotaGetter {
                count: quota_count2.clone(),
                remaining: 650,
            },
        )
        .register();
    ...
    input2.as_object_mut()?.insert(
        Value::from("validation"),
        Value::LazyObject(Arc::new(validator2)),
    );
    ...
    println!(
        "  - Stage 2 (Auth): {} checks (skipped!)",
        auth_count2.load(Ordering::SeqCst)
    );
    println!(
        "  - Stage 3 (Quota): {} checks (skipped!)",
        quota_count2.load(Ordering::SeqCst)
    );
    ...
    SchemaBuilder::new("Validator3")
        ...
        .field_immediate(
            "auth",
            AuthGetter {
                count: auth_count3.clone(),
            },
        )
        .field_immediate(
            "quota",
            QuotaGetter {
                count: quota_count3.clone(),
                remaining: 0,
            },
        )
        .register();
    ...
    input3.as_object_mut()?.insert(
        Value::from("validation"),
        Value::LazyObject(Arc::new(validator3)),
    );
    ...
    println!(
        "  - Stage 2 (Auth): {} checks",
        auth_count3.load(Ordering::SeqCst)
    );
    println!(
        "  - Stage 3 (Quota): {} checks",
        quota_count3.load(Ordering::SeqCst)
    );
