...
fn main() -> Result<()> {
    ...
    SchemaBuilder::new("NetworkInterface0")
        .field_immediate("id", NetworkInterfaceGetter { index: 0, count: access_count.clone() })
        .register();
    SchemaBuilder::new("NetworkInterface1")
        .field_immediate("id", NetworkInterfaceGetter { index: 1, count: access_count.clone() })
        .register();
    SchemaBuilder::new("NetworkInterface2")
        .field_immediate("id", NetworkInterfaceGetter { index: 2, count: access_count.clone() })
        .register();
    ...
    let interfaces_array1 = LazyArray::new(
        ...
        NetworkInterfacesIndex { count: access_count1.clone() },
    );
    ...
    properties1.as_object_mut()?.insert(Value::from("networkProfile"), network_profile1);

    let mut input1 = Value::new_object();
    input1.as_object_mut()?.insert(Value::from("properties"), properties1);
    ...
    println!("📊 Total accesses: {} (fetched all 3 interfaces)", access_count1.load(Ordering::SeqCst));
    ...
    let interfaces_array2 = LazyArray::new(
        ...
        LazyContext::new(),
        NetworkInterfacesLength,
        NetworkInterfacesIndex { count: access_count2.clone() },
    );
    ...
    properties2.as_object_mut()?.insert(Value::from("networkProfile"), network_profile2);

    let mut input2 = Value::new_object();
    input2.as_object_mut()?.insert(Value::from("properties"), properties2);
    
    engine.set_input(input2);
    let result2 = engine.eval_query("data.aliases.has_nic_in_resourcegroup(\"resourceGroups/rg\")".to_string(), false)?;
    
    println!("\n✅ Has NIC in resource group 'rg': {}", 
        result2.result.len() > 0 && result2.result[0].expressions.len() > 0);
    println!("📊 Total accesses: {} (early exit after finding match!)", access_count2.load(Ordering::SeqCst));
    ...
    let interfaces_array3 = LazyArray::new(
        ...
        LazyContext::new(),
        NetworkInterfacesLength,
        NetworkInterfacesIndex { count: access_count3.clone() },
    );
    ...
    properties3.as_object_mut()?.insert(Value::from("networkProfile"), network_profile3);

    let mut input3 = Value::new_object();
    input3.as_object_mut()?.insert(Value::from("properties"), properties3);
    ...
    println!("📊 Total accesses: {} (only fetched index 0!)", access_count3.load(Ordering::SeqCst));
