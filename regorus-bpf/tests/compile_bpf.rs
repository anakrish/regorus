...
//!
//! [`compiles_egress_object_against_live_btf`] compiles `bpf/egress.bpf.c` to a
//! BPF ELF object using `clang` + a `vmlinux.h` generated from the running
...
//! load/enforcement path and skips when the process lacks the privilege to load
//! BPF programs (the common case in unprivileged CI / dev containers).
...
#[test]
fn compiles_egress_object_against_live_btf() {
    ...
    let out = std::env::temp_dir().join(format!("regorus_bpf_{}", std::process::id()));
    std::fs::create_dir_all(&out).unwrap();
    let vmlinux_h = out.join("vmlinux.h");
    let obj = out.join("egress.bpf.o");
    ...
    let status = Command::new(&clang)
        ...
        .arg(bpf_dir())
        .arg("-c")
        .arg(bpf_dir().join("egress.bpf.c"))
        .arg("-o")
        .arg(&obj)
        .status()
        .expect("run clang");
    assert!(status.success(), "clang failed to compile egress.bpf.c");
    ...
    assert!(
        bytes
            .windows(b"cgroup/connect4".len())
            .any(|w| w == b"cgroup/connect4"),
        "object is missing the cgroup/connect4 program section"
    );
...
fn load_into_kernel_is_gated_on_privilege() {
    // Loading and enforcing the program requires CAP_BPF (and CAP_NET_ADMIN to
    // bind it to a cgroup). In unprivileged environments we cannot do this, so
    // this test only asserts the precondition and skips otherwise. The full
    // load + cgroup-bind path is exercised in privileged integration runs.
