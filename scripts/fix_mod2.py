#!/usr/bin/env python3
"""Complete rewrite of mod.rs: replace all Z3 solver interaction with SmtProblem-based code."""

FILE = "src/rvm/analysis/mod.rs"

# Read the current file to preserve type definitions and lines_to_constraints
with open(FILE) as f:
    old = f.read()

NEW = r'''// Copyright (c) Microsoft Corporation.
...
use regorus_smt::problem::{SmtProblem, SmtStatus};
...
fn maybe_render_smt(problem: &SmtProblem, config: &AnalysisConfig) -> Option<String> {
    if config.dump_smt {
        Some(regorus_smt::render::render_smtlib2(problem))
    } else {
        None
    }
}
...
#[cfg(feature = "z3-analysis")]
fn interpret_result(
    check: &regorus_smt::problem::SmtCheckResult,
    plan: &[PathExtraction],
    warnings: Vec<String>,
    ...
    match check.status {
        ...
        SmtStatus::Unsat => AnalysisResult {
            satisfiable: false,
            input: None,
            warnings,
            solver_smt,
...
pub fn policy_diff(
    ...
    problem.add_extraction("r1_matches", r1_matches, regorus_smt::expr::SmtSort::Bool, true);
    let r2_match_idx = problem.extractions.len();
    problem.add_extraction("r2_matches", r2_matches, regorus_smt::expr::SmtSort::Bool, true);
...
pub fn generate_test_suite(
        ...
        match check.status {
            SmtStatus::Sat => {
                ...
                for (i, lk) in all_lines.iter().enumerate() {
                    ...
                    if let Some(regorus_smt::problem::SmtValue::Bool(true)) =
    ...
    if condition_coverage && !pc_conditions.is_empty() {
        ...
        for (_lk, cond) in &line_conditions {
            cond_problem.add_extraction("line_cond", cond.clone(), regorus_smt::expr::SmtSort::Bool, true);
        }
        ...
        for (_lk, recs) in &cond_by_line {
            ...
            cond_problem.add_extraction("cond_val", rec.condition.clone(), regorus_smt::expr::SmtSort::Bool, true);
        }
...
print(f"Remaining z3:: in code: {len(remaining)}")
