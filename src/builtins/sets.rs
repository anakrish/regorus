...
use crate::builtins;
use crate::builtins::utils::{ensure_n_args, ensure_set};
...
pub(crate) fn intersection(expr1: &Expr, expr2: &Expr, v1: Value, v2: Value) -> Result<Value> {
...
pub(crate) fn union(expr1: &Expr, expr2: &Expr, v1: Value, v2: Value) -> Result<Value> {
    let s1 = ensure_set("union", expr1, v1)?;
    let s2 = ensure_set("union", expr2, v2)?;
    Ok(Value::from_set(s1.union(&s2).cloned().collect()))
}

pub(crate) fn difference(expr1: &Expr, expr2: &Expr, v1: Value, v2: Value) -> Result<Value> {
