...
//! - [`Object::iter_sorted`] / [`Set::iter_sorted`] are **sorted by
//!   `Value::Ord`**. Use these whenever deterministic order is required
//!   (serialization, snapshots, hashing, `Debug`, the `object.keys` builtin,
//!   RVM↔interpreter parity).
//!
//! ## Resumable iteration
//!
//! Use `Object::cursor` / `Object::next` (and the `Set` equivalents) when
//! callers must yield mid-iteration and resume later (e.g. the RVM
//! `IterationState`). The cursor types are crate-internal so future storage
//! variants can change their resume-state representation. Use plain `iter()` /
