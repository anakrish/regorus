...
fn redetect_sub_resource_arrays(entries: &BTreeMap<String, ResolvedEntry>) -> Vec<String> {
    let mut sub_resource_arrays = Vec::new();
    for entry in entries.values() {
...
mod tests {
    ...
    fn test_overwrite_removes_stale_sub_resource_arrays() {
        ...
        assert!(
            fw.sub_resource_arrays.iter().any(|s| s == "rules"),
        ...
        assert!(
            !fw2.sub_resource_arrays.iter().any(|s| s == "rules"),
