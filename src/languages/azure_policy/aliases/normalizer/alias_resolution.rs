...
use super::super::obj_map::{
    collision_safe_key, is_root_field_collision, obj_contains, obj_get, obj_insert,
...
pub fn apply_alias_entries(
    ...
    let sub_resource_arrays = &aliases.sub_resource_arrays;
    ...
    for (source_lc, target_lc) in &agg.array_renames_normalize {
        if !obj_contains(result, target_lc.as_str()) {
            if let Some(val) = obj_get(result, source_lc.as_str()).cloned() {
