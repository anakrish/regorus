...
use super::super::obj_map::{
    obj_get, obj_get_mut, obj_insert, set_nested_in_btree, set_nested_lowercased, ObjMap,
};
...
/// array chain).
pub fn apply_element_remap(result: &mut ObjMap, remap: &ElementRemap) {
...
fn remap_at_depth_in_btree(
    ...
    let Some(nav) = array_chain.get(depth) else {
        remap_deep_field_in_btree(btree, source_field, target_field);
        return;
    };
...
fn remap_deep_field_in_btree(
    ...
    set_nested_in_btree(btree, &segments, val, true);
}
...
fn remap_deep_field(obj: &mut ObjMap, source: &str, target: &str) {
