...
pub fn normalize_value(value: &Value, field_path: &str, sub_arrays: Option<&Vec<String>>) -> Value {
    match value {
        Value::Array(arr) => {
...
fn is_sub_resource_array(field_path: &str, sub_arrays: Option<&Vec<String>>) -> bool {
    sub_arrays.is_some_and(|arrays| arrays.iter().any(|a| a.eq_ignore_ascii_case(field_path)))
}
