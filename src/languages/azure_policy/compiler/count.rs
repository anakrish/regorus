...
impl Compiler {
                ...
                if suffix.as_ref().is_some_and(|s| s.contains("[*]")) {
                    if let Some(binding) = self.resolve_count_binding(&field_path)? {
                        if let Some(outer_prefix) = &binding.field_wildcard_prefix {
                            let wildcard_dot = format!("{}[*].", outer_prefix);
                            if let Some(inner_path) = field_path.strip_prefix(&wildcard_dot) {
        ...
        let any_result = match count_node {
            ...
            CountNode::Field { span, field, .. } => {
                ...
                if suffix.as_ref().is_some_and(|s| s.contains("[*]")) {
                    if let Some(binding) = self.resolve_count_binding(&field_path)? {
                        if let Some(outer_prefix) = &binding.field_wildcard_prefix {
                            let expected_prefix = format!("{}[*].", outer_prefix);
                            if let Some(inner_path) = field_path.strip_prefix(&expected_prefix) {
        ...
        for binding in self.count_bindings.iter().rev() {
            ...
            if let Some(prefix) = &binding.field_wildcard_prefix {
                ...
                if field_path == prefix
                    || field_path.starts_with(&prefix_dot)
                    || field_path == wildcard_prefix
                    || field_path.starts_with(&wildcard_dot)
                {
                    return Ok(Some(binding.clone()));
                }
            }
    ...
    pub(super) fn compile_from_binding(
        ...
        if let Some(prefix) = &binding.field_wildcard_prefix {
            ...
            let prefix_dot = format!("{}.", prefix);
            if let Some(suffix) = field_path.strip_prefix(&prefix_dot) {
                return self.compile_suffix_from_binding(binding.current_reg, suffix, span);
            }

            let wildcard_dot = format!("{}[*].", prefix);
            if let Some(suffix) = field_path.strip_prefix(&wildcard_dot) {
                return self.compile_suffix_from_binding(binding.current_reg, suffix, span);
            }
        }
        ...
        let resolve_for_key = |compiler: &mut Self, candidate: &str| -> Result<Option<u8>> {
            for binding in compiler.count_bindings.iter().rev() {
                if let Some(name) = &binding.name {
                    ...
                    let name_dot = format!("{}.", name);
                    if let Some(suffix) = candidate.strip_prefix(&name_dot) {
                ...
                if let Some(prefix) = &binding.field_wildcard_prefix {
                    ...
                    let prefix_wildcard_dot = format!("{}[*].", prefix);
                    if let Some(suffix) = candidate.strip_prefix(&prefix_wildcard_dot) {
                        return compiler
