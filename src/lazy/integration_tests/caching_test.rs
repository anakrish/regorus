...
fn test_field_caching() -> anyhow::Result<()> {
    ...
    builder = builder.field_immediate("cacheable", CountingGetter {
...
fn test_different_instances_dont_share_cache() -> anyhow::Result<()> {
    ...
    builder = builder.field_immediate("field", CountingGetter {
        count: fetch_count.clone(),
    });
    builder.register();
...
fn test_deferred_values_not_cached_until_materialized() -> anyhow::Result<()> {
    ...
    builder = builder.field_deferred("deferred_field", DeferredGetter {
