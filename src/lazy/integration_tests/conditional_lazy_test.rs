...
fn test_conditional_field_access() -> anyhow::Result<()> {
    ...
    builder = builder.field_immediate("expensive", ExpensiveGetter {
        count: expensive_count.clone(),
    });
    builder = builder.field_immediate("cheap", CheapGetter {
    ...
    input1.as_object_mut()?.insert(Value::from("type"), Value::from("cheap"));
    ...
    if let Some(cheap_value) = lazy_obj.get_field("cheap")? {
        input1.as_object_mut()?.insert(Value::from("cheap"), cheap_value);
    }
...
fn test_short_circuit_evaluation() -> anyhow::Result<()> {
    ...
    builder = builder.field_immediate("field1", Field1Getter {
        count: field1_count.clone(),
    });
    builder = builder.field_immediate("field2", Field2Getter {
