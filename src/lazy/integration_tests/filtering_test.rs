...
fn test_lazy_array_filter_comprehension() -> anyhow::Result<()> {
    ...
    impl IndexGetter for TestIndexGetter {
        fn get_at(&self, _ctx: &LazyContext, index: usize) -> Result<Option<Value>> {
            ...
            obj.as_object_mut()?
                .insert(Value::from("score"), Value::from(index % 100));
    ...
    input
        .as_object_mut()?
        .insert(Value::from("data"), Value::LazyArray(Arc::new(lazy_arr)));
...
fn test_lazy_array_exists_with_complex_condition() -> anyhow::Result<()> {
    ...
    impl IndexGetter for TestIndexGetter {
        fn get_at(&self, _ctx: &LazyContext, index: usize) -> Result<Option<Value>> {
            ...
            obj.as_object_mut()?.insert(
                Value::from("type"),
                Value::from(if index % 2 == 0 { "even" } else { "odd" }),
            );
            obj.as_object_mut()?
                .insert(Value::from("value"), Value::from(index));
    ...
    input
        .as_object_mut()?
        .insert(Value::from("items"), Value::LazyArray(Arc::new(lazy_arr)));
    ...
    let accesses = access_count.load(Ordering::SeqCst);
    std::eprintln!(
        "Complex filter early exit: accessed {} items out of 500",
        accesses
    );
...
fn test_lazy_set_membership() -> anyhow::Result<()> {
    ...
    input
        .as_object_mut()?
        .insert(Value::from("numbers"), Value::LazySet(Arc::new(lazy_set)));
    ...
    let accesses = access_count.load(Ordering::SeqCst);
    std::eprintln!(
        "Set membership check: accessed {} items to find 42",
        accesses
    );
