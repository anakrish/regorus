...
fn test_lazy_array_early_exit() -> anyhow::Result<()> {
    ...
    impl IndexGetter for TestIndexGetter {
        fn get_at(&self, _ctx: &LazyContext, index: usize) -> Result<Option<Value>> {
            ...
            obj.as_object_mut()?.insert(Value::from("name"), Value::String(format!("item_{}", index).into()));
    ...
    std::eprintln!("Lazy array early exit: accessed {} items out of 1000", accesses);
    assert!(accesses <= 10, "Expected <= 10 accesses (early exit), got {}. This is the key optimization!", accesses);
...
fn test_lazy_set_early_exit() -> anyhow::Result<()> {
    ...
    let accesses = access_count.load(Ordering::SeqCst);
    std::eprintln!("Lazy set early exit: accessed {} items out of 500", accesses);
    assert!(accesses <= 15, "Expected <= 15 accesses (early exit), got {}. This proves lazy set iteration!", accesses);
