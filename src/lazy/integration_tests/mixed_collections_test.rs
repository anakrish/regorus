...
fn test_lazy_array_in_eager_object() -> anyhow::Result<()> {
    ...
    obj.as_object_mut()?.insert(Value::from("lazy_data"), Value::LazyArray(Arc::new(lazy_arr)));
    obj.as_object_mut()?.insert(Value::from("eager_data"), Value::from("immediate"));
    
    // Access eager field - should not trigger lazy fetch
    assert_eq!(obj[&Value::from("eager_data")].as_string()?.as_ref(), "immediate");
...
fn test_eager_array_of_lazy_objects() -> anyhow::Result<()> {
    ...
    for i in 0..3 {
        ...
        let lazy_item = LazyObject::new(TypeId::new(&alloc::format!("Item{}", i)), LazyContext::new());
