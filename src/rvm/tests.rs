...
mod tests {
    ...
    fn compile_and_run_rvm(
        ...
        std::println!(
            "Debug: Generated {} instructions",
            compiled_program.instructions.len()
        );
            ...
            match (&case.want_result, &case.want_error) {
                (Some(expected_result), None) => {
                    ...
                    match compile_and_run_rvm(&compiled_policy, &case.query) {
                        Ok(actual_result) => {
                            ...
                            std::println!("Expected: {:?}", expected_results);
                            std::println!("Actual: {:?}", actual_results);
                ...
                (None, Some(expected_error)) => {
                    ...
                    match compile_and_run_rvm(&compiled_policy, &case.query) {
                        ...
                        Err(actual_error) => {
                            let actual_error_str = format!("{}", actual_error);
    ...
    fn yaml_test(file: &str) -> anyhow::Result<()> {
        yaml_test_impl(file)
    }

    #[test]
    fn test_yaml_suite() -> anyhow::Result<()> {
        yaml_test("tests/rvm/cases/test_cases.yaml")
    }
}
