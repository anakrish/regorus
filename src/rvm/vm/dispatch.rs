...
impl RegoVM {
            ...
            | Mod { dest, left, right } => {
                ...
                if a == &Value::Undefined || b == &Value::Undefined {
                    ...
                    let result = match instruction {
                        Add { .. } => self.add_values(a, b)?,
                        Sub { .. } => self.sub_values(a, b)?,
                        Mul { .. } => self.mul_values(a, b)?,
                        Div { .. } => self.div_values(a, b)?,
                        Mod { .. } => self.mod_values(a, b)?,
                        _ => unreachable!(),
                    };
    ...
    fn execute_comparison_instruction(
        ...
        match instruction {
            ...
            And { dest, left, right } => {
                let a = self.to_bool(&self.registers[left as usize]);
                let b = self.to_bool(&self.registers[right as usize]);
            ...
            Not { dest, operand } => {
                let a = self.to_bool(&self.registers[operand as usize]);
                self.registers[dest as usize] = Value::Bool(!a);
                Ok(InstructionOutcome::Continue)
            }
