...
fn case_registry(
    case: &TestCase,
    file_registry: &Option<AliasRegistry>,
) -> Result<Option<AliasRegistry>> {
    if let Some(ref inline) = case.aliases_json {
        let mut reg = AliasRegistry::new();
        reg.load_from_json(inline)?;
        return Ok(Some(reg));
    }
    Ok(file_registry.clone())
}
...
fn resolve_aliases(
    ...
    if let Some(ref subs) = case.sub_resource_arrays {
        return Some(ResolvedAliases {
            ...
            sub_resource_arrays: subs.clone(),
...
fn run_yaml_test(file: &str) -> Result<()> {
    ...
    for case in &test.cases {
        ...
        let registry = case_registry(case, &file_registry)?;
        ...
        if let Some(ref expected) = case.expected_normalized {
            ...
            let actual = if case.use_registry_api {
                normalizer::normalize(&input, registry.as_ref(), api_ver)
            } else {
        ...
        if let Some(ref expected) = case.expected_denormalized {
            let expected = to_regorus(expected);
            let actual = if case.use_registry_api {
                denormalizer::denormalize(&input, registry.as_ref(), api_ver)
            } else {
        ...
        if case.round_trip {
            let (normalized, denormalized) = if case.use_registry_api {
                let n = normalizer::normalize(&input, registry.as_ref(), api_ver);
                let d = denormalizer::denormalize(&n, registry.as_ref(), api_ver);
                (n, d)
            } else {
        ...
        if case.reverse_round_trip {
            let (denormalized, renormalized) = if case.use_registry_api {
                let d = denormalizer::denormalize(&input, registry.as_ref(), api_ver);
                let n = normalizer::normalize(&d, registry.as_ref(), api_ver);
                (d, n)
            } else {
