...
//!
//! - `StringKeyed`: uses `hashbrown::HashMap<SmolStr, Value>`. O(1) lookup by `&str`.
//! - `Mixed`: uses `hashbrown::HashMap<Value, Value>` for non-string keys. Ord computed on the fly.
...
pub enum ObjectMap {
    ...
    StringKeyed(HashMap<SmolStr, Value>),
...
impl ObjectMap {
    ...
    pub fn from_pairs(pairs: impl IntoIterator<Item = (Value, Value)>) -> Self {
        ...
        if all_string_keys {
            let map: HashMap<SmolStr, Value> = entries
                .into_iter()
                .map(|(k, v)| {
                    let key = match k {
                        Value::String(s) => SmolStr::new(s.as_ref()),
                        _ => unreachable!(),
                    };
                    (key, v)
                })
    ...
    pub fn from_regorus_btree(
        ...
        if all_string_keys {
            let map: HashMap<SmolStr, Value> = m
                ...
                .map(|(k, v)| {
                    let key = match k {
                        regorus::Value::String(s) => SmolStr::new(s.as_ref()),
                        _ => unreachable!(),
                    };
    ...
    pub fn get_str(&self, key: &str) -> Option<&Value> {
        match self {
            ...
            ObjectMap::Mixed(m) => {
                // Must wrap in Value::String to look up in BTreeMap<Value, Value>.
    ...
    /// Insert a string key.
    pub fn insert_str(&mut self, key: SmolStr, value: Value) {
        match self {
            ObjectMap::StringKeyed(m) => {
                m.insert(key, value);
            }
            ObjectMap::Mixed(m) => {
                m.insert(Value::String(Arc::from(key.as_str())), value);
            }
        }
    }
    ...
    pub fn insert(&mut self, key: Value, value: Value) {
        match key {
            Value::String(ref s) => {
                self.insert_str(SmolStr::new(s.as_ref()), value);
            }
            _ => {
    ...
    fn ensure_mixed(&mut self) {
        if let ObjectMap::StringKeyed(m) = self {
            let mixed: HashMap<Value, Value> = m
                .drain()
                .map(|(k, v)| (Value::String(Arc::from(k.as_str())), v))
                .collect();
...
pub enum ObjectMapIter<'a> {
    String(hashbrown::hash_map::Iter<'a, SmolStr, Value>),
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            ObjectMapIter::String(it) => {
                let (k, v) = it.next()?;
                Some((Value::String(Arc::from(k.as_str())), v))
            }
