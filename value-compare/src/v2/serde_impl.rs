...
//! Serde Serialize/Deserialize for v2::Value.
//!
...
/// Wrapper that serializes a Value with sorted object keys.
/// Use this when deterministic output is required (e.g. `json.marshal`).
...
impl Serialize for Value {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        match self {
            ...
            Value::Object(o) => {
                // Unsorted iteration — fast path (no allocation).
                // For StringKeyed, iterate directly over Arc<str> keys.
...
impl<'de> Visitor<'de> for ValueVisitor {
    ...
    fn visit_map<A: MapAccess<'de>>(self, mut map: A) -> Result<Value, A::Error> {
        // JSON keys are always strings, so we can always build StringKeyed.
        let mut obj = hashbrown::HashMap::with_capacity(map.size_hint().unwrap_or(0));
        while let Some((key, value)) = map.next_entry::<String, Value>()? {
            obj.insert(Arc::<str>::from(key.as_str()), value);
        }
        Ok(Value::Object(Arc::new(ObjectMap::StringKeyed(obj))))
    }
}

// ---------------------------------------------------------------------------
//  Interned deserialization — reuses Arc<str> for repeated keys
// ---------------------------------------------------------------------------

/// Newtype wrapper that deserializes a `Value` with key interning enabled.
///
/// When deserializing many objects with the same keys (e.g., an array of
/// similar JSON objects), the interner ensures each unique key string is
/// allocated only once. Subsequent encounters reuse the existing `Arc<str>`.
///
/// ```ignore
/// let interned: v2::Interned = serde_json::from_str(json)?;
/// let value: v2::Value = interned.0;
/// ```
