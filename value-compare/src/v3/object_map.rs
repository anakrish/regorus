...
//! - Always maintains a `HashMap<Arc<str>, Value>` for string keys (fast `get_str`).
//! - Optional `HashMap<Value, Value>` for non-string keys (rare in practice).
//! - Precomputed hash at construction time, using order-independent hashing
//!   (wrapping_add of per-entry hashes via `DefaultHasher`).
//! - `PartialEq` checks cached hash first for fast mismatch rejection.
//! - `Hash` returns the precomputed hash — O(1).
//! - `Ord` is retained for Rego comparison operators but is not on the hot path.
...
/// Uses `DefaultHasher` (SipHash with fixed seed) so the result is deterministic.
...
impl ObjectMap {
    ...
    pub fn get(&self, key: &Value) -> Option<&Value> {
        match key {
            Value::String(s) => self.strings.get(s.as_ref()),
    ...
    pub fn insert_str(&mut self, key: Arc<str>, value: Value) {
        let kv = Value::String(Arc::clone(&key));
        // If replacing, remove old entry's hash contribution.
        if let Some(old_val) = self.strings.get(&key) {
...
pub struct ObjectMapIter<'a> {
    string_iter: hashbrown::hash_map::Iter<'a, Arc<str>, Value>,
...
impl<'a> Iterator for ObjectMapIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some((k, v)) = self.string_iter.next() {
            return Some((Value::String(Arc::clone(k)), v));
        }
...
impl PartialEq for ObjectMap {
    fn eq(&self, other: &Self) -> bool {
        ...
        // Point lookups in the other map — O(n) average.
        for (k, v) in self.strings.iter() {
            match other.strings.get(k.as_ref()) {
                Some(ov) if v == ov => {}
                _ => return false,
            }
        }
        ...
        } else if other.other.is_some() {
            // self has no `other` but other does — checked via len already.
        }
        true
    }
}
