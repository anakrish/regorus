...
//! Serialize / Deserialize for v3::Value.
//!
//! Differences from v2:
//! - Sets deserialize into `HashSet<Value>` instead of `BTreeSet<Value>`.
//! - `SortedValue` serialization sorts HashSet elements for deterministic output.
...
impl Serialize for Value {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        match self {
            ...
            Value::Set(s) => {
                // Serialize as array (JSON has no set type).
...
impl<'de> Visitor<'de> for ValueVisitor {
    ...
    fn visit_map<A: MapAccess<'de>>(self, mut map: A) -> Result<Value, A::Error> {
        ...
        while let Some((key, value)) = map.next_entry::<String, Value>()? {
            obj.insert_str(Arc::from(key.as_str()), value);
        }
