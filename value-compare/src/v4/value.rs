...
/// A Rego value with 16-byte footprint via thin `ArcStr` string pointer.
///
/// Key differences from v3:
/// - `String` variant uses `arcstr::ArcStr` (8 bytes) instead of `Arc<str>` (16 bytes).
/// - Total enum size: 16 bytes (down from 24 in v3).
/// - All v3 features retained: HashSet for sets, precomputed object hash.
#[derive(Debug, Clone)]
pub enum Value {
    /// JSON null.
    Null,
    ...
    /// String value. `ArcStr` for cheap cloning (8-byte thin pointer).
    String(ArcStr),
...
// ---------------------------------------------------------------------------
//  Eq — fast paths with ArcStr::ptr_eq and hash-based rejection
// ---------------------------------------------------------------------------
...
impl Value {
    /// Convert from regorus::Value (baseline) to v4::Value.
