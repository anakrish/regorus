...
impl ObjectMap {
    ...
    /// Build a compact object from sorted keys and values.
    /// Keys must be provided in insertion order; values in the same order.
    pub(crate) fn from_schema_values(schema: Arc<Schema>, values: Box<[Value]>) -> Self {
        ...
        for (k, v) in schema.keys.iter().zip(values.iter()) {
            let kv = Value::from_arcstr(k.clone());
    ...
    pub fn get(&self, key: &Value) -> Option<&Value> {
        if let Some(s) = key.as_str_ref() {
            ...
            match &self.repr {
                ObjectRepr::Compact(_) => None, // compact objects only have string keys
    ...
    pub fn iter_sorted(&self) -> Vec<(Value, &Value)> {
        match &self.repr {
            ObjectRepr::Compact(c) => {
                // Keys are already sorted in schema — just zip.
                c.schema
                    .keys
                    .iter()
                    .zip(c.values.iter())
                    .map(|(k, v)| (Value::from_arcstr(k.clone()), v))
                    .collect()
            }
            ObjectRepr::Map(_) => {
        ...
        match self {
            ObjectMapIter::Compact { idx, obj } => {
                if *idx < obj.values.len() {
                    let k = Value::from_arcstr(obj.schema.keys[*idx].clone());
...
// ---------------------------------------------------------------------------
//  Eq — fast rejection via cached hash + fast path for same-schema
// ---------------------------------------------------------------------------
...
impl PartialEq for ObjectMap {
    fn eq(&self, other: &Self) -> bool {
        ...
        // General path: compare entry by entry.
